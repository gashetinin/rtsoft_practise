...
static noinline int i2cdev_ioctl_rd2b(struct i2c_client *client,unsigned long arg, int offset)
{
	...
	msg[1].buf = rdwr_arg.msgs->buf;
	msg[1].len = rdwr_arg.msgs->len;
	...
	do {
		...
		if (status == 2)
			status = rdwr_arg.msgs->len;
		dev_dbg(&client->dev, "read %zu --> %d (%ld)\n",rdwr_arg.msgs->len, status, jiffies);
		pr_alert("Status %d\n",status);
		if (status == rdwr_arg.msgs->len)
			return rdwr_arg.msgs->len;
...
static noinline int i2cdev_ioctl_wr2b(struct i2c_client *client,unsigned long arg, int offset)
{
	...
	*/
	...
	do
	{
		...
		if (status == 2)
			status = rdwr_arg.msgs->len;
		dev_dbg(&client->dev, "write %zu --> %zd (%ld)\n",rdwr_arg.msgs->len, status, jiffies);
		pr_alert("Status %d\n",status);
		if (status == rdwr_arg.msgs->len)
			return rdwr_arg.msgs->len;
